...
{
    isPhoneNumber = localUid.startsWith("/org/freedesktop/Telepathy/Account/ring/");
...
RecipientList::const_iterator RecipientList::begin() const
{
    return m_recipients.begin();
}

RecipientList::const_iterator RecipientList::end() const
{
...
QDebug &operator<<(QDebug &debug, const CommHistory::RecipientList &list)
{
    ...
    foreach (const Recipient &r, list.recipients()) {
        debug << r.localUid() << " " << r.remoteUid() << " | "
              << r.contactId() << " " << r.contactName() << ", ";
    }
    return debug.space();
}
